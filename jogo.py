...
def botao(texto, funcao):
    ...
    # CONTINUAR AGORA É AUTOMÁTICO
    if texto == "➡️ Continuar":

        callback = create_proxy(lambda event: funcao(None))
    ...
    def clicar(event):
        ...
        # Remove os outros botões depois da escolha
        botoes.innerHTML = ""

        funcao(event)

    b.onclick = create_proxy(clicar)
